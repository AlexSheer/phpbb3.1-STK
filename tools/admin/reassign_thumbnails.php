...
class reassign_thumbnails
{
	...
	function run_tool()
	{
		global $config, $db, $user, $cache, $template, $phpEx;
		...
		if (!$total)
		{
			...
			trigger_error('NO_THUMBNAILS_TO_REBUILD', E_USER_WARNING);
		}

		$list = sprintf($user->lang['NEED_TO_PROCESS'], $total) . '<br />';
		...
		for ($i = 0; $i < count($batch); $i++)
		{
			...
			if (file_exists($upload_path . $batch[$i]['physical_filename']))
			{
				...
				{
					$output[] = $user->lang['REBUILT'] . $batch[$i]['physical_filename'] . ' ' . $user->lang['THUMB'] . ' '. $thumb_file_name;
					$thumbs[] = $batch[$i]['attach_id'];
				}
				else
				{
					$output[] = $user->lang['NO_NEED_REBUILT'] . $batch[$i]['physical_filename'];
				}
			}
			else
			{
				$output[] = $user->lang['SOURCE_UNAVAILABLE'] . $batch[$i]['physical_filename'];
			}
		}
		...
		if (empty($batch))
		{
			...
			trigger_error('REASSIGN_THUMBNAILS_FINISHED');
		}
		...
		trigger_error('' . $user->lang['REASSIGN_THUMBNAILS_PROGRESS'] . '<br />' . $list . implode("<br />", $output) . '');
	}
	...
	function count_total_images()
	{
		...
		if (!$group_id)
		{
			$sql = 'SELECT group_id
				FROM ' . EXTENSION_GROUPS_TABLE . '
				WHERE group_name = \'' . $user->lang['IMAGES'] . '\'';
			$result = $db->sql_query($sql);
			$group_id = (int) $db->sql_fetchfield('group_id');
			$db->sql_freeresult($result);
			if (!$group_id)
			{
				trigger_error($user->lang['NO_EXTENSIONS_GROUP'], E_USER_WARNING);
			}
		}
		...
		if (!$extensions)
		{
			trigger_error($user->lang['NO_EXTENSIONS'], E_USER_WARNING);
		}
